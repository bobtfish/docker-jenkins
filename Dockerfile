...
FROM ubuntu:12.04
RUN apt-get update && apt-get install -y build-essential openjdk-6-jdk maven python-dev libz-dev libcurl4-gnutls-dev libsasl2-dev rubygems git && gem install puppet && ln -sf /jenkins /root/.jenkins && ln -s /usr/lib/jvm/java-6-openjdk-amd64/jre/lib/amd64/server/libjvm.so /usr/lib/libjvm.so ; mkdir /jenkins
